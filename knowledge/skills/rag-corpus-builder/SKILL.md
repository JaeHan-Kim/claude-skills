...
3. **Choose chunk boundaries.** Cut on the structure the source already has: headings, API endpoints, functions/classes, decisions, runbook steps, tickets, or policy clauses. Do not run embedding-based "semantic chunking" to find boundaries — on retrieval benchmarks it has not repaid its cost over simple splitting (Qu et al., NAACL 2025 Findings). Where structure is missing, split fixed-size with overlap. Size matters more than the method: keep a chunk to one answerable point, well inside the embedding window (`documents_windowed` above zero at index time means the chunks are too coarse), and let the parent note carry the surrounding context — search returns one result per note and `knowledge_get` opens the whole note.
...
5. **Situate every chunk with a `context` sentence.** A passage cut out of a note loses the words that name it ("the company's revenue grew 3%" — which company, which quarter?). Write one or two sentences, 50–100 tokens, saying where the chunk sits in its note and what it is about, in the vault's own vocabulary, and store it in `context`. The indexer prepends it to the text that is both matched and embedded (Contextual Retrieval, Anthropic 2024: −35% retrieval failures from embeddings alone, −49% with BM25). Say what the chunk is, not what the whole note is — a generic note summary pasted onto every chunk was found to add little. The chunk also inherits its parent note's title and catalog vocabulary automatically, so do not copy those into `context`.
6. **Preserve identity and citations.** Derive stable chunk IDs from a stable note/source ID plus a durable heading or block anchor where possible. Each chunk should be traceable back to the exact source region using heading paths, line ranges, page numbers, ticket IDs, or URLs where available.
7. **Handle duplicates and conflicts.** Keep canonical chunks for repeated content, preserve aliases, and mark conflicting or stale sources rather than blending them into one unsupported statement.
8. **Create eval queries.** Reuse stable question IDs from `_knowledge/questions.jsonl` when present. Add retrieval tests for direct lookup, synonym/operator-language lookup, source-symbol lookup, comparison, multi-document synthesis, freshness-sensitive questions, and negative queries that should not retrieve irrelevant content. Keep retrieval relevance results distinct from `_knowledge/question-results.jsonl`, which grades whether retrieved evidence can establish the answer.
9. **Emit importable artifacts.** Produce the default portable layout unless the user requested a specific target. If a vector store is requested, keep the portable artifacts and add an ingestion note for that store.
...
```json
{
  ...
  "context": "One or two sentences situating this chunk within its note",
  "text": "retrieval text",
