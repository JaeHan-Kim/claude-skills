...
## 9. 외부 시스템 대조 — Hindsight (2026-09-24)

[Hindsight](https://github.com/vectorize-io/hindsight)(Vectorize.io)는 "학습하는 에이전트 메모리"를
표방하는 시스템으로, 이 플러그인과 **검색 엔진 층이 거의 같고 쓰기 층이 완전히 다릅니다.** 대조 자체를
여기 남깁니다 — 다음 사람이 같은 조사를 반복하지 않도록, 그리고 "우리가 독립적으로 도달한 것"과
"업계가 이미 정리해 둔 것"을 구분할 수 있도록.

**이 대조의 한계**: 공개 README와 문서만 읽었고 코드를 돌리거나 읽지 않았습니다. LongMemEval SOTA
주장도 재현하지 않았습니다. 아래 "우리 대응"은 검증된 우리 쪽 사실이고, "Hindsight" 칸은 그들의 서술입니다.

### 9.1 대조표

| 개념 | Hindsight | 우리 대응 | 판정 |
|---|---|---|---|
| 하이브리드 검색 — semantic + BM25 병렬 후 융합 | 4갈래 병렬(semantic / keyword / graph / temporal) | FTS5 컬럼별 3갈래 + semantic, §4.1·4.3 | **일치** |
| Reciprocal Rank Fusion | 명시 | §4.3, 동일 알고리즘 | **일치** |
| 크로스인코더 리랭커 | shortlist 재정렬 | §4.7, `--reranker-url` 부착식 | **일치**, 단 우리는 **천장을 미리 잰다**(`recall@50 − recall@10`). 그들은 천장을 말하지 않는다 |
| 엔티티/관계 링크를 검색에 쓴다 | graph 전략(entity·causal 링크) | §4.8 relation 참여자 승격 | **일치**, 단 우리가 더 엄격 — **선언된 `participants`만**, 동시출현은 관계를 지명할 수 있어도 확정하지 못한다(§3.1) |
| 근거는 정확한 인용으로 보관 | observation마다 exact quote + proof count | §6.2 "연 것만 인용", §5.2 인용 정밀도/재현율 | **일치** |
| 벤치마크 규율 | LongMemEval, 외부 재현 | §5.3–5.5 holdout·부호 검정·1회 1변경 | **일치**, 단 우리가 더 엄격 — 부호 검정 p<0.05 없이는 승자를 부르지 않고, 측정 조건이 다르면 `incomparable`로 거부 |
| **시간 축** | retain이 temporal data를 추출하고, 시간 범위 필터가 4갈래 중 하나 | **없음** | **차이 → §9.2** |
| **믿음의 강도** | observation은 덮어쓰지 않고 **강화·약화·확장**된다 | 노트는 고쳐 쓰거나 안 쓰거나. 근거가 재수집 사이에 누적되지 않음 | **차이 → §9.3** |
| **질문에 대한 상주 답변** | mental model = "한 번 정의한 질문에 대해 시스템이 답을 쓰고 저장하고 배경에서 다시 쓴다. 읽기는 DB 읽기 — 검색도 LLM 호출도 없음" | `questions.jsonl`이 **이미 그 질문 집합**이다. 다만 게이트와 평가 셋으로만 쓴다 | **차이 → §9.4. 이 대조에서 가장 값어치 있는 항목** |
| 에이전트 자신의 경험을 기억한다 | world facts / experiences 구분, 실행하며 계속 retain | 볼트는 **출처에서** 오프라인으로 만든다 | **차이지만 채택 대상 아님 → §9.5** |
| 성향 특성(skepticism, literalism, empathy)이 추론을 바꾼다 | bank마다 부여 | 없음 | **채택 대상 아님 → §9.5** |
| 저장소 | PostgreSQL + pgvector 필수 | SQLite + `hash` 기본, 의존성 0 | **차이, 우리 쪽이 의도적** — 런타임 의존성 금지(ROADMAP "안 하기로 유지") |

### 9.2 시간 축이 없다 (차이, 미해결)

- **이론**: 지식에는 유효 구간이 있다. 폐기된 결정 노트와 현행 결정 노트는 내용이 아니라 **시점**으로
  구분되고, 시점을 모르면 검색은 둘을 같은 자격으로 내놓는다. Hindsight는 이것을 검색 전략 하나로
  격상시켰다(4갈래 중 temporal).
- **우리 상태**: 노트에 유효 구간이 없다. `decision` 노트 유형이 있는데도 "이 결정은 언제부터 언제까지
  참인가"를 담을 자리가 없고, 대체된 결정은 대체한 결정과 **검색상 구별되지 않는다.**
- **판정**: 실재하는 격차. 다만 **지금 손댈 레버가 아니다** — ROADMAP 2026-08-31 측정이 "회수는 더 이상
  병목이 아니다"(94문항 중 0건 인용은 1건)라고 판정했고, 시간 축은 회수 쪽 레버다. 이미 병목이 아닌 곳에
  레버를 더 붙이는 것이 v1.5~v1.11에서 반복한 실수다.
- **먼저 잴 것**: 오답 중 "폐기된 노트를 현행으로 인용" 유형이 몇 건인가. 0에 가까우면 이 항목은 닫는다.

### 9.3 믿음의 강도가 없다 (차이, 판단 보류)

- **이론**: 새 증거는 기존 믿음을 **대체**하는 것이 아니라 강화·약화·확장한다. 덮어쓰면 "왜 그렇게 믿는가"의
  이력이 사라지고, 약한 근거 하나가 강한 근거 열을 조용히 지운다.
- **우리 상태**: `relation` 노트의 `evidence_by_participant`가 가장 가까운 것이지만, 그것은 **한 번 쓰는**
  근거지 재수집 사이에 누적되는 카운트가 아니다.
- **판정**: 우리 볼트는 사람이 소스를 읽고 쓰는 것이라 "증거가 시간에 걸쳐 도착한다"는 전제 자체가 약하다.
  Hindsight는 에이전트가 계속 retain하므로 그 전제가 성립한다. **전제가 다른 기능을 전제 없이 가져오면
  비용만 남는다** — 보류.

### 9.4 `questions.jsonl`은 이미 mental model의 질문 절반이다 (차이, 여기가 레버)

이 대조에서 유일하게 **지금 값어치가 있는** 항목입니다.

- **Hindsight의 수**: 질문을 한 번 정의하면 시스템이 답을 써서 저장하고, 뱅크가 배우는 만큼 배경에서
  다시 쓴다. 읽을 때는 검색도 LLM 호출도 없는 **DB 읽기**다.
...
- **재구성**: ROADMAP Phase 4(생성층)는 지금 "`knowledge-query` 위에 챗봇을 얹는다"로 적혀 있고, 그래서
  ...
  1. **산출물과 평가 대상이 같은 객체가 된다.** 지금 eval은 "이 질문에 답할 때 정답 노트를 인용하는가"를
     재는데, 그 답변 자체가 저장되면 재는 것과 파는 것이 분리되지 않는다.
  2. **재작성 트리거가 이미 있다.** v1.15.0의 콘텐츠 해시 기반 증분 재색인이 "무엇이 바뀌었는가"를 안다.
     바뀐 노트를 `required_note_ids`로 갖는 질문만 다시 쓰면 된다 — 배경 재작성의 스코프가 공짜로 나온다.
  3. **측정된 병목 위에 정확히 앉는다.** 2026-08-31 판정은 병목이 회수가 아니라 **생성층의 인용 규율**
     (0.644 / 0.476)이라고 했다. 상주 답변은 생성층 산출물이고, 인용 정밀도(v1.16.0)가 이미 그 채점기다.
- **판정**: Phase 4의 게이트("recall 0.8+ 전에는 시작 안 함")는 **챗봇에 대해서는 여전히 맞지만 상주 답변에
  대해서는 과하다.** recall 0.803은 이미 넘겼고, 답이 근거 없이 지어내는지는 인용 정밀도가 잡는다.
  다음 측정(같은 94문항 재실행) 결과를 보고 결정한다 — 그 측정이 이 항목의 게이트다.

### 9.5 채택하지 않을 것

- **에이전트 자신의 경험(experiences)을 1급 기억으로 둔다** — 이 플러그인의 모든 노트는 출처가 있는
  주장이고(§1.1), 답변은 연 것만 인용한다(§6.2). "에이전트가 겪은 일"은 그 신뢰 등급이 아니다. 한 볼트
  안에 두 등급을 섞으면 `required_note_ids`가 무엇을 보증하는지 말할 수 없게 된다.
- **성향 특성(disposition traits)** — 추론을 성향으로 기울이는 것은 answerability 근거주의의 정확한 반대다.
  "회의적으로 읽어라"는 근거가 바뀌지 않는데 답이 바뀌는 장치다.
- **LongMemEval을 우리 지표로 삼기** — 대화형 장기 기억 벤치마크이고, 우리 대상은 볼트별 컴피턴시 질문이다.
  그들의 SOTA 주장은 우리 쪽으로 이전되지 않는다.
...
| 20 | Vectorize.io, [*Hindsight*](https://github.com/vectorize-io/hindsight) (2026) | §9 |
