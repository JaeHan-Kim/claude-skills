...
- **v1.7.1 — 베타 벤치가 공유 엔진에서 찾은 수정 셋**: graph-beta의 첫 e2e 라운드
  (`graph-beta/scripts/bench/`)가 이 엔진 코드를 실제 세션으로 돌려 유닛 스위트에 없던 결함
  셋을 쳤고, stable 벤치 런 둘도 첫 번째를 재현했습니다. (1) 자신을 `claude-opus-5[1m]`(fresh
  agent 선택기에 없는 컨텍스트 변형)로 보고한 구동 세션이 `plan`에서 `native host cannot select
  model`로 막혔습니다; 호스트 자신의 모델은 이제 정의상 선택 가능합니다. (2) 실행 기본값은
  티어 이름(`sonnet`), 호스트는 ID(`claude-sonnet-5`)를 선언하는데 검사는 문자열을 비교해서,
  모든 `implement` 노드가 실패 노드 0개인 채 `vendor-failure`가 될 수 있었고 출구는 두 번째
  `graph_open`뿐이었습니다. 이제 티어를 선언 목록에 대응시키고, 선언되지 않은 티어는 호스트
  모델로 돌되 라우팅 이유에 치환을 적습니다. (3) 조립 결과를 거부한 `gate:goal`은 탓한 서브골이
  재시도된 뒤 다시 판정되지 않았습니다 — 수정이 들어간 채 런이 막혔습니다; 이제 서브골 재시도가
  살아 있는 게이트들 위에 `gate:goal:N`을 열고 `report`를 그 뒤로 옮기며, 재시도 서브골의
  briefing은 목표 게이트의 이유와 gaps(그리고 재시도가 잃어버리던 실패한 test의 checks)를
  싣습니다. 테스트는 수정과 함께 포트. 벤치: 이 버전 직전의 stable이 패키지 4개 요청을 plain
  세션의 6× 비용·시간으로 9/9 완주; 같은 요청의 graph-beta 매니저는 다른 위상(런 넷 + 매니저)
  이라 그쪽에서 따로 읽습니다.
