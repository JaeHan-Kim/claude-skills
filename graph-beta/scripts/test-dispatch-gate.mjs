...
function project(config) {
  ...
  if (config !== null) writeFileSync(join(dir, '.claude', 'graph-beta-dispatch.json'), JSON.stringify(config));
  return dir;
}
...
test('a gated write with nothing open is denied, and the message says what to call', () => {
  ...
  try {
    ...
    assert.match(r.stderr, /remove \.claude\/graph-beta-dispatch\.json/, 'a gate must say how to get out of it');
  } finally { rmSync(dir, { recursive: true, force: true }); }
});
