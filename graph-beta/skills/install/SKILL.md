...
name: install
description: >-
  Use when installing graph-beta into a project: team.json defaults, the dispatch gate,
  conventions, and the coexistence check against the stable graph plugin. Not for running
  a task; use graph-beta:orchestrate.
scenarios:
  - "이 프로젝트에 graph-beta 설치해줘"
  - "team.json이랑 dispatch 게이트 설정해줘"
  - "Install graph-beta into this project"
...
remain plugin-owned and are updated with the `graph-beta` plugin. The deterministic file work —
...
2. Run:
   ...
   server or is enabled with the same `graph_*` tool surface — and let the user choose to
...
Prefer the marketplace plugin. Its `.mcp.json` already registers the `graph-beta-engineering`
and `task-manager` stdio servers, so installation should not add project files beyond what
`install.mjs` writes. Ask the user to install or update `graph-beta@newkayak12-claude-skills`
...
servers exposing the same `graph_*` tools make routing ambiguous — that is exactly what
...
until `.claude/graph-beta-dispatch.json` exists, which `install.mjs` now writes from the
...
- Confirm the dispatch patterns and roles. Choose how to resolve a coexistence conflict
  ...
  `.claude/graph-beta-dispatch.json`, `.claude/conventions/`, and the CLAUDE.md block so the
