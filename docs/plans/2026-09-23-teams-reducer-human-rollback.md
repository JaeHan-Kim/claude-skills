...
### 0.2e 완주 (2026-09-25) — `flow: plan` 기획 런이 report에 닿았다

`idol-beta-ask1`의 PLAN 자식 런이 **완주했습니다**: 33 done / 3 failed / 5 skipped,
`dispatch:PLAN:1=done`. 카드 9장 전부 답해짐. 이것으로 `ask` 경로가 "실모델 런 한 판이 report에
닿아야 done"(feedback-real-vendor-run-before-done) 기준을 실제로 통과했습니다.

**산출물**: 문서 4개 581줄 + findings 4개 122줄.
`prd.md`(184) `domain-model.md`(93) `policies.md`(93) `scale-assumptions.md`(211).

**동시에 검증된 것 셋 (전부 이번 판이 처음):**

| | 무엇이 처음 검증됐나 |
|---|---|
| **0.27.4 파생 findings 경로** | `reduce`가 선언 8건(문서 4 + findings 4), **미선언·충돌·고아 전부 0**. 비교: `idol-plan-2`는 같은 자리에서 파일명 5종 + 고아 1건. U4는 3회차까지 재시도했는데 **고아 0** — 0.27.4가 D3 부분 완화로 주장한 것이 측정됐다 |
| **0.27.0 `reduce`** | 실런 첫 fold. 문서 집합을 실제로 읽고 보고했다 |
| **0.27.0 재판정 라운드** | `gate:goal:1=failed` → `repair:1` → `gate:goal:2=done`. "재판정이 새 라운드를 못 열던" 버그가 없다 |

**목표 게이트가 잡은 것 — 그리고 이 기능이 만든 구멍의 실제 피해:**

`gate:goal:1`이 78%로 거절하며 정확히 이렇게 말했습니다 — *"U1이 U2~U4의 결정보다 먼저 돌고
게이트까지 통과했고, 병렬 `deps: []` 분할에는 그것들을 화해시킬 스테이지가 없었다."*

`ask:U1:1`의 결정만으로 PRD가 쓰이고 게이트를 통과한 뒤, U2·U3·U4의 카드에서 **U1의 결정을 좁히거나
뒤집는 결정**이 나왔습니다. PRD는 다시 안 쓰였으므로 `prd.md`는 "2–4 미결", `policies.md`는
"2 확정"이었습니다. §0.2d에 "형제 간 질문 중복 = 사람이 두 번 답한다"로 적었던 것의 **실제 피해는
문서 간 불일치**였습니다.

`reduce`는 이것을 못 잡습니다 — 설계상 파일 층만 보고 내용 일치는 보지 않습니다(§0.1). 목표 게이트가
잡는 것이 맞고, **`repair:1`이 닫았습니다**: 네 문서를 고치고 PRD에 승계 표
(`| Decision | ask:U1:1 said | Superseded by | Now says |`)를 넣었으며, 결정마다 어느 카드가 정했는지와
형제 문서를 인용합니다. `gate:goal:2` accept 90 / gaps 0.

**판정: 계약 문구를 더 쓰지 않는다.** 엔진이 이미 갖고 있던 경로(목표 게이트 + 수리 패스)가 이 결함을
잡고 닫았습니다. §0.2d에서 검토했던 선택지 1("형제 investigate가 끝난 뒤 ask를 한 번만 열기 —
병렬성 상실")은 지금 근거가 없습니다. 재측정 조건: 수리 패스가 이 이음새를 **못** 닫는 판이 나오면
그때 다시 본다.

**품질 표본** — `prd.md`에 이 문장이 있습니다: *"D1–D10 안의 숫자 중 아직 정해지지 않은 것은 여기서
확정으로 재서술하지 않고 Open questions에 적는다."* idol-pm-2(2026-09-22)가 실패한 정확한 지점이고,
`investigate`/`draft` 계약을 다시 쓴 이유였습니다.
