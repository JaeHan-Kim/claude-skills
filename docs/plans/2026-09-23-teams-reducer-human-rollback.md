...
| **D2** | **time travel** — `waiting_human`이 설계만 있고 미구현, 롤백 개념 없음 | **진행 중 (2026-09-23, 로컬 세션)** — 1단계: 사람이 카드를 가져간다(`tm_assign`, `assignee:"human"` 핀, `waiting_human`, `tm_inbox`, 사람 제출 후 흐름 계속). `ask`/`gate:human`/롤백은 그다음 | Task 3, 4, 5 |
...
- **선행 측정** — `flow: develop` 실런 한 판으로 0.24.0 계약 3규칙의 효과를 잰다. 성공 기준은
  ...
  **idol-pm-4 (2026-09-23, 0.26.4) — 결과: shape이 처음으로 critique를 통과했다(이전 0/4).**
  ...
  런은 P1 dispatch로 진행 중 — PM 런이 패키지 dispatch에 닿은 첫 사례.
