...
### 6b. 지켜볼 사람이 없을 때 — headless 모드 (실런으로 발견, 2026-09-17)

위 표와 §10.6은 **사람이 있는 모드** 하나만 상정한다. `tm_open` 한 번 하고 끝내도 되는 이유는
사람이 `/teams:board`로 본다는 전제이기 때문이다. 그 전제가 없는 자리가 둘 있고, 둘 다 실제로
쓰인다: **벤치**와 **CI**, 그리고 일반적으로 `claude -p`로 도는 모든 세션.

거기서는 main이 `tm_open` 뒤 곧바로 끝내면 안 된다. 헤드리스 세션은 **모델이 툴 호출을 멈추는
순간 종료**되고, 종료된 세션은 보고할 대상이 없다 — leader와 드라이버는 계속 짓지만 아무도
기다리지 않는 워크스페이스에 짓는다. 첫 실벤더 런이 죽은 자리가 정확히 여기다(1분, 11턴,
"I'll check again in about four minutes" 하고 셸 sleep을 건 뒤 턴 종료; 그 런은 버려진 채로 끝까지
지어 9/9를 만들었다).

**세션을 붙잡아 둘 수 있는 것은 아직 반환되지 않은 툴 콜 하나뿐이다.** 그래서 `tm_next`는
`wait_ms`를 받아 태스크가 running을 벗어날 때까지 서버에서 블로킹하고, 호출자는 반환되는 즉시
다시 부른다. 90초 블로킹이 Claude Code의 MCP 클라이언트를 통과하는 것은 측정했다; 상한 300초,
스킬은 60초를 쓴다.

| 모드 | main이 하는 일 | 근거 |
|---|---|---|
| 사람 있음 (대화형) | `tm_open` 하고 끝. 사람이 `/teams:board` | 위 표, §10.6 |
| 사람 없음 (bench·CI·`-p`) | `tm_open` → `tm_next({wait_ms})` 반복 → complete/blocked에서 보고 | 이 절 |

**이것은 §6의 "main은 절대 드라이브하지 않는다"를 어기지 않는다.** main은 여전히 노드를 하나도
돌리지 않고 leader를 대신하지도 않는다 — 기다리기만 한다. 금지된 것은 드라이빙이지 대기가 아니다.
...
6. **main이 루프에서 빠지면 관측이 늦다.** → 보드 명령 + `/loop` watch. driver의 stream 파일은
   지금도 있다(`driver.log`).
   **이 완화책은 사람이 있을 때만 성립한다** (실런으로 확인, 2026-09-17): 벤치·CI·`claude -p`에는
   보드를 볼 사람이 없고, main이 루프에서 빠지는 순간 세션 자체가 끝나 태스크가 고아가 된다.
   헤드리스 모드의 답은 §6b — `tm_next({wait_ms})`가 서버에서 블로킹해 세션을 붙잡아 둔다.
7. **어휘.** Team/TeamLeader/Task/TaskLeader 넷 중 "Task"가 코드의 tm task와 충돌. → 외부 어휘는
   EPIC/STORY/TASK, 내부 식별자는 유지.
8. **벤더가 하나뿐일 때 독립성이 조용히 사라진다** (실런으로 발견, 2026-09-17). §3의 독립성은
   "draft는 peer 벤더로, review는 호스트에 남는다 — 아무도 따로 주선하지 않아도"라는 배치에서
   나온다. peer가 설치돼 있지 않으면(이 저장소의 벤치 arm들이 전부 그렇다) draft가 호스트로
   강등되고 저자와 리뷰어가 같은 정체성이 된다. 그때 가드는 제출을 거부하는데, 가드가 안내하는
   탈출구 둘(`policy.review`로 다른 벤더 / `team_run`에 다른 모델)은 **둘 다 그 상황에서 불가능**
   하다 — 두 번째 벤더가 없고, `team_run`은 self 라우팅 노드를 거부한다. 런은 같은 노드를 영원히
   내놓는다(실측: 드라이버 세션 셋이 연속으로 같은 자리에서 포기, 16/20 정지). → **모델 축**을
   먼저 쓴다: 저자와 같은 호스트 모델이면 리뷰어를 다른 native model로 라우팅하고, 선언된 모델이
   하나뿐이면 리뷰를 진행하되 `reviewer_independence: "unverifiable-same-host"`로 남긴다. 데드락
   대신 **가시적 저하** — 벤더 대 벤더 거부는 그대로다.
9. **단위 테스트가 이 층의 결함을 구조적으로 못 본다** (2026-09-17). 실런 한 번이 하루 만에 결함
   넷을 냈고, 그때 단위 테스트 358개는 전부 통과하고 있었다. 넷 다 테스트가 닿지 않는 자리에
   있었다: spawn 경로(`HARNESS_TEST_NO_DRIVER`가 꺼 버린다), 헤드리스 세션의 수명, 벤더 부재 시
   라우팅, 채점기의 size-S 판정. → 이 라인에서 "검증됐다"의 단위는 테스트 수가 아니라 **완주한
   실런의 수**다. 새 경로를 열면 그 경로로 실런을 한 번 돌리기 전까지 미검증으로 센다.
...
- ~~**역할별 마운트 키잉**~~ — **닫혔다(`5e8ab47`, 2026-09-17).** §3이 요구한 `{role}:{stage}`
  키잉이 `teams/mcp/mounts.mjs`에 `GRAPH_STAGE_MOUNTS_BY_KIND`로 들어갔다 — `<kind>:<stage>`로
  키를 잡고, `nodeKind(run, n)`(`graph.mjs`)으로 kind를 구한 뒤 단계-전용 표보다 **먼저** 본다.
  v0.10.1이 단계 키만 넣어 `document`의 draft가 planning의 PRD 프레이밍 프롬프트를 같이 받던
  부작용(그 파일의 코드 주석이 스스로 인정하던 것)이 실제 결함이었고, 이 커밋이 그것을 고쳤다.
  현재 항목은 `planning:draft` 하나뿐인데 그게 맞다 — 나머지 단계는 kind별로 갈릴 이유가 없어서
  단계 키에 남겨 뒀다. 이 줄은 "미배치 일감"이 아니라 이력으로 남긴다.

이 목록과 별개로, **결정은 났지만 의도적으로 미배치인 것**이 하나 더 있다:

- **`human_scope: "all"`** — §7이 "`"all"`이면 TeamLeader도 human 노드를 열 수 있다"고 적어
  뒀지만, v0.13.0은 기본값 `'leader'` 경로만 만든다(`docs/plans/2026-09-17-teams-team-v0.13.0.md`
  §0.4-3). `'all'`을 제대로 지원하려면 `broker.mjs`의 child-run 매니저 루프에 `ask`/`gate:human`
  삽입을 따로 구현해야 하고, 그건 사이징 문서 §4.2가 경고한 "v0.13.0이 11–12 unit을 넘길" 가장
  그럴듯한 원인이다(+1 unit 후보). **팀 리더 결정(2026-09-17): 미구현으로 두되 조용히 무시하지
  않는다.** v0.13.0 Task 4가 `teamconfig.mjs`의 설정 해석 단계에서 `human_scope: "all"`을
  `'leader'`로 강등하고 전용 note를 남긴다 — `tm_status`의 `team.notes`, `team_status`의
  `config_notes`(`3292a91`)로 이미 있는 경로 그대로 사용자에게 보인다. 옵션은 받아들여지는데
  값 하나만 실제로 읽히는 상태로 두지 않는 것이, 이 저장소가 이번 주에 세 번 고친 결함군
  (`f765c03`/`4999ed8`, `be83bbc`, `3292a91`)을 네 번째로 만들지 않는 유일한 방법이다.
  어느 버전에 넣을지는 정하지 않는다 — 위 둘과 마찬가지로 사용자의 결정이다.
