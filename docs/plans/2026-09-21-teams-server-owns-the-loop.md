...
### 8i. 기획에는 무언가를 읽는 스테이지가 없었다 (2026-09-22, teams 0.26.0)

8h를 쓸 때 나는 세 선택지를 내놓으면서 그중 하나가 아니라 **문제 자체를 잘못 짚고 있었다**. 0.24.0과 0.25.0에서 내가 한 것은 전부 프롬프트 텍스트를 더 쓴 것이다 — `CONTRACT.shape` 세 규칙, `PLANNING_SETGOAL`, `PRD_CONTRACT`의 Out of scope 구멍 막기. 입력이 0인 스테이지에 "더 잘 써라"고 말만 더 한 것이고, 그래서 한 번 더 재봐야 같은 답이 나올 일이었다. 사용자의 지적("계속 점수 재는게 능사일까? 충분히 했지만 퀄리티가 떨어짐")이 정확했다.

코드 한 줄이 원인을 그대로 보여준다:

```
graph.mjs   planning: { chain: ['draft', 'revise', 'gate'] }
```

`subgoal`은 `test`에서 명령을 돌리고 `qa`는 `execute`에서 케이스 세트를 실행한다. 종류마다 어딘가에서 현실과 부딪히는데 **`planning`만 그 접점이 0이다**. draft는 요청 문장만 보고 쓰고, revise는 draft가 쓴 것을 고치고, gate는 그것을 채점한다. 이 체인은 요청에 적힌 것 이상을 알 방법이 구조적으로 없다. 그러니 팬클럽 선예매 자격, 취소 수수료 기준일, 1인 한도는 지어내거나 빠뜨리는 것 외에 선택지가 없었고, 실제로 idol-pm-1은 빠뜨렸고 idol-pm-2는 "존재한다고 가정"으로 지어냈다.

**0.26.0: `investigate → draft → revise → gate`.** `investigate`는 브리핑 바깥을 읽는 유일한 스테이지다 — 프로젝트 트리, 요청이 지목·첨부한 자료, 이전 문서, 검색 도구가 실제로 닿으면 도메인의 공개 출처. 돌려주는 두 목록은 일부러 분리되어 있다: `findings`(각각 그렇게 말하는 출처와 함께)와 `unknowns`(닿은 어떤 출처도 답하지 못한 결정, 소유자와 함께). "존재한다고 가정"은 findings의 옷을 입은 unknown이다. `draft`는 findings에서 쓰고 모든 unknown을 소유자 붙은 Open question으로 들고 간다. **대부분 unknown으로 돌아온 조사는 성공한 것이다** — 그 목록이 작성자가 지어내지 못하게 막는 장치이고, 목록을 부풀리는 것만이 이 스테이지가 숨길 수 있는 유일한 실패다.

같이 드러난 구조 버그 둘 (둘 다 4단 체인이 처음이라 드러난 것):

- `authorStage`가 `chain[0]`이었다 → revise가 작성자가 아니라 조사자를 상대로 독립성을 검사하게 되어, 결국 작성자가 자기 초안을 퇴고할 수 있었다. 종류가 `author`를 명시하게 고쳤다.
- `taskTicketState`가 모든 체인을 `[author, mid, gate]` 3단으로 분해했다 → 4단에서는 `revise`가 gate 자리에 들어가, 기획 TASK가 revise가 끝나는 순간 DONE으로 보고되고 실패한 gate는 REJECTED가 될 수 없었다. 위치로 유도하게 고쳤다.

**남은 자리 — human을 노드로 쓰는 phase.** 조사가 닿는 것은 소스에 적힌 것까지다. 선예매 자격처럼 "소스에 없고 사람만 아는" 칸은 조사로 못 채운다. 지금은 그것이 소유자 붙은 Open question으로 서고, 그게 지어내는 것보다는 정직하다. 다음 자리는 그 목록을 사람에게 묻는 `ask:human` 스테이지다 — `tickets.mjs`가 `no WAITING_USER (no human executor exists yet)`라고 적어둔 그 빈칸이고, 순서가 핵심이다: **조사가 못 채운 것만 묻는다.** 정해야 할 것은 셋 — human 노드의 만료(마감이 지나면 `unanswered`로 닫히고 무인 실행이 계속된다), 질문이 사람에게 도달하는 경로(`board`가 "human-inbox counts … tooling this round didn't build"로 남겨둔 것), 그리고 조사 사정권(자식 런의 웹 도구 가용 여부는 드라이버 기본값이라 미확인).

**아직 안 된 것은 그대로다.** shape은 여전히 critique를 한 번도 통과한 적이 없고(0/4), 어떤 PM 런도 첫 패키지 dispatch에 도달한 적이 없다. 0.26.0은 8h의 3번(재측정) 앞에 놓인 변경이지, 그것을 대체하지 않는다. 다만 이제 측정할 가치가 있는 것이 생겼다 — 조사 없이 잰 숫자는 스테이지의 능력이 아니라 입력의 부재를 재고 있었다.
