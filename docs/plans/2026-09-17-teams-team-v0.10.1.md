# teams (team) v0.10.1 — planning/qa kind + 페르소나·mounts + entry 스킬 2개 구현 계획

> Produced by write:writing-plans. Owner for execution routing: planning:executing-plans.
> Steps use checkbox (`- [ ]`) syntax. 설계 근거: `2026-09-17-teams-team.md` §3·§11.
...
`teams:plan`/`teams:qa`를 신설한다. EPIC 흐름(§2)에 planning을 shape 앞에, qa를 integrate
...
(`teams:plan`, `teams:qa`), 단위/통합 테스트, bench 요청 파일 2개(`plan-flat`/`qa-flat`).
...
**전제 사실 (조사로 확인):**
- `teams/mcp/graph.mjs`의 `KINDS`는 오늘 `subgoal`(implement→test→gate)과
...
- `teams/mcp/mounts.mjs`(설계 문서가 "broker.mjs"라 부른 것과 달리 실제로는 별도 파일이다 —
...
- `pm` 플러그인에는 `prd-development` 스킬이 있고 그 `template.md`는 10절 스켈레톤(Executive
  ...
  이 항목은 그냥 뺐다.
- `teams/skills/{develop,document}/SKILL.md`가 entry 스킬의 유일한 선례다. 둘 다 `tm_open({flow})`을
  고정하고 `../orchestrate/{SKILL.md,references/loop.md}`를 그대로 따르는 얇은 래퍼 — 새 entry 스킬
  둘도 같은 모양이다.
- `teams/scripts/bench/requests/`의 `*-flat.txt`는 평문 한 단락 요청서다. `tinyq`
...
5. 팀 리더의 지시는 `GRAPH_STAGE_SKILLS`/`GRAPH_STAGE_MOUNTS`를 "broker.mjs"에서 찾으라고 했지만
   실제로는 `teams/mcp/mounts.mjs`에 있다(broker.mjs는 그 값을 가져다 쓰지 않고,
...
**Files:** modify `teams/mcp/graph.mjs`, create `teams/scripts/test-graph.mjs`
...
**Pass bar:** `node --test teams/scripts/test-graph.mjs` 6개 통과.
...
```
- [ ] 2: `node --test teams/scripts/test-graph.mjs` → import 대상이 없어 6개 모두 fail 확인
- [ ] 3: `teams/mcp/graph.mjs`의 `KINDS` 선언(56-75행)을 바꾼다 — 기존 `subgoal`/`document`는 그대로 두고
  닫는 `};` 앞에 두 항목을 추가:
```js
export const KINDS = {
  subgoal: {
...
```
- [ ] 4: `node --test teams/scripts/test-graph.mjs` 6개 통과 확인
- [ ] 5: `git add teams/mcp/graph.mjs teams/scripts/test-graph.mjs && git commit -m "feat(teams): KINDS.planning/qa, VERDICT_FIELD.execute, FLOWS.plan/qa"`

---

### Task 2: `prompts.mjs` — `CONTRACT.revise`/`CONTRACT.cases`/`CONTRACT.execute`
**Files:** modify `teams/mcp/prompts.mjs`, modify `teams/scripts/test-prompts.mjs`
...
**Pass bar:** `node --test teams/scripts/test-prompts.mjs` 전체 통과(회귀 0), 새로 추가한 4개 포함.

- [ ] 1: 실패하는 테스트를 `teams/scripts/test-prompts.mjs` 끝에 추가한다(기존 `tmpProject`/`baseRun`/
...
```
- [ ] 2: `node --test teams/scripts/test-prompts.mjs` → 새 4개가 fail 확인(나머지는 그대로 통과)
- [ ] 3: `teams/mcp/prompts.mjs`의 `CONTRACT` 테이블에서 `review:` 항목과 `gate:` 항목 사이에 세 항목을 끼운다:
```js
...
```
- [ ] 4: 전체 통과 확인(회귀 0)
- [ ] 5: `git add teams/mcp/prompts.mjs teams/scripts/test-prompts.mjs && git commit -m "feat(teams): CONTRACT.revise/cases/execute - the two new kinds no longer fall back to CONTRACT.implement"`

---

### Task 3: `mounts.mjs` — `GRAPH_STAGE_MOUNTS.draft`/`.cases`
**Files:** modify `teams/mcp/mounts.mjs`, modify `teams/scripts/test-mounts.mjs`
**Interfaces:** produces 두 개의 `GRAPH_STAGE_MOUNTS` 항목. Task 1·2와 파일이 겹치지 않아 완전히 독립적으로 병행 가능.
**Pass bar:** `node --test teams/scripts/test-mounts.mjs` 전체 통과(회귀 0), 새로 추가한 2개 포함.

- [ ] 1: 실패하는 테스트를 `teams/scripts/test-mounts.mjs`의 "stage-mounted MCP tools" 섹션 끝에 추가한다:
```js
...
```
- [ ] 2: `node --test teams/scripts/test-mounts.mjs` → 새 2개가 fail 확인
- [ ] 3: `teams/mcp/mounts.mjs`의 `GRAPH_STAGE_MOUNTS` 선언을 바꾼다:
```js
...
```
- [ ] 4: 전체 통과 확인(회귀 0)
- [ ] 5: `git add teams/mcp/mounts.mjs teams/scripts/test-mounts.mjs && git commit -m "feat(teams): mount draft/cases with an advisory MCP tool"`

---

### Task 4: `teams:plan` entry 스킬
**Files:** create `teams/skills/plan/SKILL.md`
...
description: >-
  ...
  requirement write-up to run through the teams harness — "기획서 그래프로", "PRD 작성해서
...
description: >-
  ...
  and executed against work that already exists - to run through the teams harness —
...
**Files:** create `teams/scripts/bench/requests/plan-flat.txt`, create `teams/scripts/bench/requests/qa-flat.txt`
**Interfaces:** 소비 없음. 둘 다 이미 있는 `fixtures/tinyq` 픽스처를 겨냥한다(새 픽스처 없음) —
다른 모든 태스크와 완전히 병행 가능.
**Pass bar:** 두 파일이 존재하고 비어 있지 않다. `ls teams/scripts/bench/fixtures/tinyq/src` 로
참조한 파일 경로(`queue.mjs`/`worker.mjs`/`retry.mjs`)가 실제로 있음을 확인.

- [ ] 1: `teams/scripts/bench/requests/plan-flat.txt`
```
...
```
- [ ] 2: `teams/scripts/bench/requests/qa-flat.txt`
```
...
```
- [ ] 3: `ls teams/scripts/bench/fixtures/tinyq/src/queue.mjs teams/scripts/bench/fixtures/tinyq/src/worker.mjs teams/scripts/bench/fixtures/tinyq/src/retry.mjs` — 셋 다 존재 확인
- [ ] 4: `git add teams/scripts/bench/requests/plan-flat.txt teams/scripts/bench/requests/qa-flat.txt && git commit -m "test(teams): bench requests for the planning and qa kinds, against the existing tinyq fixture"`

---

### Task 7: `broker.mjs` — revise도 review와 같은 정체성 분리 강제 + 통합 테스트
**Files:** modify `teams/mcp/broker.mjs`, modify `teams/scripts/test-broker.mjs`
...
적용되는 동작.
**Pass bar:** `node --test teams/scripts/test-broker.mjs` 전체 통과(회귀 0), 새로 추가한 3개 포함.
...
```
- [ ] 2: `node --test teams/scripts/test-broker.mjs` → 새 3개가 fail 확인(둘째 테스트는 KINDS가
...
- [ ] 3: `teams/mcp/broker.mjs`의 `reviewIndependence` 함수를 바꾼다:
```js
...
- [ ] 4: 전체 통과 확인(회귀 0)
- [ ] 5: `git add teams/mcp/broker.mjs teams/scripts/test-broker.mjs && git commit -m "feat(teams): planning/qa kinds reach the broker - revise gets review's identity-separation guard, execute reassigns like test"`
...
- [ ] 3: patch 범프: `teams/.claude-plugin/plugin.json`과 `.claude-plugin/marketplace.json`의 teams
  ...
  personas/skills/MCP mounts, and teams:plan/teams:qa entry skills." 추가
- [ ] 4: README `## Status` 맨 위, KOR `## 상태` 맨 위에 한 줄씩 prepend(기존 v0.10.0 항목은 그 아래 그대로 둔다):
  - README: `- **v0.10.1 — planning and qa kinds**: two new KINDS entries, draft→revise→gate and
    ...
    entry skills, teams:plan and teams:qa, that pin the flow the same way develop/document
...
- **소비/생산 연결**: Task 7은 Task 1(`KINDS`/`VERDICT_FIELD`/`FLOWS`)·Task 2(`CONTRACT`)·Task 3
  ...
  `KINDS.planning`/`qa`가 없으면 `team_open`이 그 kind로 확장할 스펙 자체를 만들 수 없다). Task 8은
  Task 7까지 전부의 위에 선다.
