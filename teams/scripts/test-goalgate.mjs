...
//
//   node --test teams/scripts/test-goalgate.mjs
...
async function openRun(c, cwd, extra = {}) {
  const r = await c.call('team_open', { request: 'r', cwd, vendor: 'self', ...extra });
  return r.run_id;
}
...
async function throughGoalGate(c, cwd, runId) {
  ...
  const f = dirty(cwd);
  await c.call('team_submit', { run_id: runId, cwd, node_id: 'implement:U1:1', payload: ok({ changed_files: [f], handoff: 'built' }) });
...
test('goal_judges:1 reproduces the single-node shape every earlier run had', async () => {
  await withRun(async ({ c, cwd, runId }) => {
    ...
    const v = await c.call('team_submit', { run_id: runId, cwd, node_id: 'gate:goal:1', payload: ok({ accept: true, match_pct: 95 }) });
    assert.equal(v.state, 'done');
    assert.equal(v.repaired, undefined);
    const after = await c.call('team_next', { run_id: runId, cwd });
...
test('both judges accept, but one is below threshold: consensus refuses on the number', async () => {
  await withRun(async ({ c, cwd, runId }) => {
    await throughGoalGate(c, cwd, runId);
    await c.call('team_submit', { run_id: runId, cwd, node_id: 'gate:goal:1', payload: ok({ accept: true, match_pct: 95 }) });
    const v = await c.call('team_submit', { run_id: runId, cwd, node_id: 'gate:goal:1b', payload: ok({ accept: true, match_pct: 60 }) });
...
test('every judge accepts at or above the threshold: consensus accepts, no repair', async () => {
  await withRun(async ({ c, cwd, runId }) => {
    await throughGoalGate(c, cwd, runId);
    await c.call('team_submit', { run_id: runId, cwd, node_id: 'gate:goal:1', payload: ok({ accept: true, match_pct: 95 }) });
    const v = await c.call('team_submit', { run_id: runId, cwd, node_id: 'gate:goal:1b', payload: ok({ accept: true, match_pct: 92 }) });
    assert.equal(v.state, 'done');
    assert.equal(v.repaired, undefined);
    const st = await c.call('team_status', { run_id: runId, cwd });
...
test('repair accepted: a fresh goal-gate round follows it', async () => {
  await withRun(async ({ c, cwd, runId }) => {
    await throughGoalGate(c, cwd, runId);
    await c.call('team_submit', { run_id: runId, cwd, node_id: 'gate:goal:1', payload: ok({ accept: false, match_pct: 40, gaps: ['g1'], reason: 'short' }) });
    let nx = await c.call('team_next', { run_id: runId, cwd });
    assert.deepEqual(nx.ready.map((n) => n.node_id), ['repair:1']);

    const f = dirty(cwd, 'b.txt');
    writeFileSync(join(cwd, 'b.txt'), 'seam fix\n');
    const v = await c.call('team_submit', { run_id: runId, cwd, node_id: 'repair:1', payload: ok({ changed_files: [f], handoff: 'fixed the seam' }) });
    assert.equal(v.state, 'done');
    ...
    await c.call('team_submit', { run_id: runId, cwd, node_id: 'gate:goal:2', payload: ok({ accept: true, match_pct: 96 }) });
    nx = await c.call('team_next', { run_id: runId, cwd });
    assert.deepEqual(nx.ready.map((n) => n.node_id), ['report']);
  }, { goal_judges: 1 });
});
...
test('a repair round that closes the same gaps twice stalls: the run proceeds to report on partial work', async () => {
  await withRun(async ({ c, cwd, runId }) => {
    await throughGoalGate(c, cwd, runId);
    await c.call('team_submit', { run_id: runId, cwd, node_id: 'gate:goal:1', payload: ok({ accept: false, match_pct: 40, gaps: ['same gap'], reason: 'short' }) });
    let nx = await c.call('team_next', { run_id: runId, cwd });
    assert.deepEqual(nx.ready.map((n) => n.node_id), ['repair:1']);
    await c.call('team_submit', { run_id: runId, cwd, node_id: 'repair:1', payload: ok({ changed_files: [], handoff: 'tried' }) });
    nx = await c.call('team_next', { run_id: runId, cwd });
    assert.deepEqual(nx.ready.map((n) => n.node_id), ['gate:goal:2']);

    const v = await c.call('team_submit', { run_id: runId, cwd, node_id: 'gate:goal:2', payload: ok({ accept: false, match_pct: 40, gaps: ['same gap'], reason: 'still short' }) });
    assert.equal(v.state, 'failed');
...
test('team_retry({repair:true}) forces a repair round when auto_reassign is off', async () => {
  await withRun(async ({ c, cwd, runId }) => {
    await throughGoalGate(c, cwd, runId);
    const v = await c.call('team_submit', { run_id: runId, cwd, node_id: 'gate:goal:1', payload: ok({ accept: false, match_pct: 40, gaps: ['g'], reason: 'short' }) });
    assert.equal(v.repaired, undefined, 'auto_reassign is off: nothing opens on its own');
    let nx = await c.call('team_next', { run_id: runId, cwd });
    assert.equal(nx.state, 'blocked');

    const rt = await c.call('team_retry', { run_id: runId, cwd, repair: true });
    ...
    await c.call('team_submit', { run_id: runId, cwd, node_id: 'repair:1', payload: ok({ changed_files: [], handoff: 'fixed' }) });
    await c.call('team_submit', { run_id: runId, cwd, node_id: 'gate:goal:2', payload: ok({ accept: true, match_pct: 95 }) });
    const again = await c.call('team_retry', { run_id: runId, cwd, repair: true });
