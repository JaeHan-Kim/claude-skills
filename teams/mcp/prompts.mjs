...
files[] is the markdown path this subgoal WRITES and nothing else - the spec check rejects any other path there, and reduce reads files[] as what each subgoal declared it wrote. Each planning subgoal opens with an investigate stage that reads the sources before anything is written, so name what that document's investigator ought to open - the parts of the tree, the attached or referenced material, the prior documents - in the subgoal's own sources[] field. A document whose subject nothing in reach can speak to is still worth naming: its investigation will come back as unknowns with owners, which is a real deliverable and the honest alternative to a section someone invented.`
