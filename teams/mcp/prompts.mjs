...
Each planning subgoal opens with an investigate stage that reads the sources before anything is written, so files[] should name what that document's investigator ought to open - the parts of the tree, the attached or referenced material, the prior documents - and not only the path to write. A document whose subject nothing in reach can speak to is still worth naming: its investigation will come back as unknowns with owners, which is a real deliverable and the honest alternative to a section someone invented.`
...
Read what is actually on disk under the run's output paths, not what the subgoals said they wrote - a handoff is a claim. Compare that against every subgoal's declared files[]. Four things come back: a declared path that does not exist, a file nobody declared, one path two subgoals both wrote, and an artifact left behind by a superseded attempt.
An orphan is the one you may act on without asking: when a retried subgoal's later attempt wrote to a different path than its first, the first attempt's file is stale and misleading to the reader - say so in "orphans" and remove it, naming the removal in "changed_files". Everything else you report and leave alone; a collision between two live subgoals is a shaping defect, and repairing it here would hide it from the gate that should see it.
Do not write, rewrite or improve the content of any artifact. This stage is about the set, never about the prose.
