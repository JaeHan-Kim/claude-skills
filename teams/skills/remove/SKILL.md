...
name: remove
description: >-
  Use when removing teams from a project: team.json, the dispatch gate, the CLAUDE.md
...
1. List what will be removed unconditionally: `.claude/team.json`,
   `.claude/teams-dispatch.json`, the fenced `<!-- teams:begin -->` block in
...
4. Read the JSON report and pass it through as-is: `marker-error` means `CLAUDE.md` had
   unmatched `teams:begin`/`end` markers and was left untouched; `kept-harness` means the
