...
in a bare `team_open({vendor: "auto"})` call until you name it (`vendor: "your-vendor"`)
...
inherits it. `team_next` reports the chosen `model` per ready node.
...
STAGE_SKILLS). `team_open` has the same mechanism for the graph engine's own stages —
...
skills only ever apply inside a subgoal chain. `team_open({skills: {plan: [...]}})`
overrides one stage without touching the rest; `skills: false` runs every one of those
stages on its contract alone. `team_open({mounts: {...}})` is the same switch for the
